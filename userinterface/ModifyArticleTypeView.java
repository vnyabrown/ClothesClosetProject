...
public class ModifyArticleTypeView extends View
{
    ...
    public void processAction(Event event) {
        ...
        if(text.getText() == null || text.getText().isEmpty()) {
            ...
            text.requestFocus();
        } else if(modificationField.getValue().equals("Barcode Prefix") && !checkBarcodePrefix(text.getText())) {
            text.requestFocus();
        }
        else {
            String[] values = new String[2];
            if(modificationField.getValue().equals("Barcode Prefix")) {
                values[0] = formatBarcodePrefix(text.getText());
            } else {
...
//	 	pubYear.setText((String)myModel.getState("pubYear"));
    ...
    public void updateState(String key, Object value)
    {
        ...
        if(key.equals("successfulModify")) {
            displayMessage("Article type successfully modified.");
        } else if (key.equals("unsuccessfulModify")) {
            displayErrorMessage("Error modifying article type.");
        } else {
            System.out.println("else in updateState in ModifyCrticleTypeView");
        }
    }
